...
export function EmailAnalyzer() {
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    try {
      // Validate that we have either email or QR data
      const hasEmail = email && email.trim() !== ""
      const hasQrData = qrText || qrPreview // Allow if we have decoded QR text OR just a preview
      
      if (!hasEmail && !hasQrData) {
        setError("Please enter an email address or upload a QR code")
        setLoading(false)
        return
      }

      // Send payload - include qrText if available, or indicate QR preview exists
      const payload = { 
        email: hasEmail ? email.trim() : "", 
        qrText: qrText || (qrPreview ? "QR_UPLOADED_NO_TEXT" : null)
      }
      ...
      if (!response.ok) {
        ...
        console.error('Analyze API error response:', response.status, text)
      ...
      // Provide more detailed error messaging when possible
      const message = (err as Error)?.message || String(err)
      setError(`Failed to analyze: ${message}. Make sure the dev server is running and /api/analyze is reachable.`)
  ...
  // QR file handler: uses BarcodeDetector when available, otherwise keeps preview
  const handleQrFile = async (file?: File | null) => {
    ...
    try {
      ...
      // Try to decode using BarcodeDetector (Chromium-based browsers)
      // Fallback: we still upload qrText=null so server knows a QR was provided
      // and can apply heuristics
      // @ts-ignore
      ...
      // If decoding failed, clear qrText (server will still know a QR was uploaded via preview)
      setQrText(null)
    } catch (e) {
        ...
        <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="lg:col-span-1">
            <form onSubmit={handleAnalyze} className="space-y-4">
              ...
              <div>
                <label className="block text-xs sm:text-sm font-semibold mb-2">Upload QR Code (optional)</label>
                <input
                  type="file"
                  accept="image/*"
                  onChange={async (e) => {
                    const f = e.target.files ? e.target.files[0] : null
                    await handleQrFile(f || undefined)
                  }}
                  className="w-full text-xs"
                />
                {qrPreview && (
                  <img src={qrPreview} alt="QR preview" className="mt-2 w-32 h-32 object-cover rounded-md border" />
                )}
                {qrText && (
                  <p className="text-sm mt-2 text-muted-foreground">Detected QR content: <code className="break-all">{qrText}</code></p>
                )}
              </div>
              ...
              <button
                type="submit"
                disabled={loading || ((!email || email.trim() === "") && !qrText && !qrPreview)}
